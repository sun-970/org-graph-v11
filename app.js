...
function circleSticker(img) {
  ...
  const vg = ctx.createRadialGradient(256, 220, 80, 256, 256, 238);
  vg.addColorStop(0, "rgba(0,0,0,0)");
  vg.addColorStop(0.72, "rgba(0,0,0,0)");
  vg.addColorStop(1, "rgba(8, 12, 18, 0.22)");
  ctx.fillStyle = vg;
  ctx.fillRect(0, 0, 512, 512);
  ctx.restore();
  strokeRing(ctx, 256, 256, 238);
  const texture = new THREE.CanvasTexture(canvas);
  texture.colorSpace = THREE.SRGBColorSpace;
  return texture;
}
...
void main() {
  ...
  float cs = cos(spin);
  float sn = sin(spin);
  vec3 rp = vec3(cs * vObjPos.x + sn * vObjPos.z, vObjPos.y, -sn * vObjPos.x + cs * vObjPos.z);
  float lon = atan(rp.x, rp.z);
  float lat = rp.y;
  float mer = smoothstep(0.03, 0.0, abs(sin(lon * 7.0)));
  float par = smoothstep(0.035, 0.0, abs(sin(lat * 10.0)));
  float grid = max(mer, par) * 0.09 * (0.25 + 0.75 * (1.0 - facing));

  vec3 keyL = normalize(vec3(0.46, 0.78, 0.42));
  vec3 fillL = normalize(vec3(-0.62, 0.18, -0.38));
  vec3 spinL = normalize(vec3(cs * 0.72, 0.28, sn * 0.72));
  float ndKey = max(dot(n, keyL), 0.0);
  float ndFill = max(dot(n, fillL), 0.0);
  float ndSpin = max(dot(n, spinL), 0.0);
  float hemi = n.y * 0.5 + 0.5;
  vec3 globe = ballColor * (0.1 + 0.32 * hemi + 0.9 * ndKey + 0.22 * ndFill + 0.28 * ndSpin);
  globe += vec3(0.62, 0.78, 0.92) * grid;
  vec3 h = normalize(V + keyL);
  float spec = pow(max(dot(n, h), 0.0), 42.0);
  float spec2 = pow(max(dot(n, normalize(V + spinL)), 0.0), 90.0);
  globe += vec3(1.0, 0.95, 0.82) * (spec * 0.42 + spec2 * 0.28);
  globe += vec3(0.86, 0.72, 0.38) * fresnel * 0.7;

  vec3 up = abs(V.y) < 0.96 ? vec3(0.0, 1.0, 0.0) : vec3(1.0, 0.0, 0.0);
  vec3 T = normalize(cross(up, V));
  vec3 B = cross(V, T);
  vec2 uv = vec2(dot(n, T), dot(n, B)) / (0.58 + 0.22 * facing);
  float r = length(uv);
  vec2 texUV = clamp(uv * 0.5 + 0.5, 0.0, 1.0);
  float mask = smoothstep(0.98, 0.72, r) * smoothstep(0.1, 0.38, facing);
  vec4 port = texture2D(portrait, texUV);
  float wrapShade = mix(0.58, 1.0, facing);
  float edgeDark = mix(1.0, 0.68, smoothstep(0.28, 1.0, r));
  vec3 face = port.rgb * (0.7 + 0.3 * ndKey) * wrapShade * edgeDark;
  vec3 col = mix(globe, face, mask * port.a);
  col += vec3(0.86, 0.74, 0.4) * selected * 0.2;
...
async function buildPeople() {
  ...
  PEOPLE.forEach((person, i) => {
    ...
    const color = new THREE.Color().setHSL(hue / 360, 0.28, 0.16);
